...
import {
    ...
    ArrowLeft, DoorOpen, Activity, XCircle, Eye, EyeOff, ChevronLeft, RotateCcw, MessageSquare, Settings, AlertTriangle
} from 'lucide-react';
...
import { supabase, registerAppointmentPatient, updateUserPassword, deleteAccount } from "./lib/supabaseClient";
...
const DoctorDashboard = () => {
    ...
    // Redirect if doctor no longer exists (e.g., account was deleted)
    useEffect(() => {
        if (doctorId && !allDoctors.find(d => d.id === doctorId)) {
            localStorage.removeItem('selectedDoctorId');
            navigate('/login');
        }
    }, [allDoctors, doctorId, navigate]);
    ...
    return (
        <div className="flex flex-col h-[100dvh] bg-[#F8FAFC] font-sans text-slate-900 overflow-hidden">
                ...
                <>
                    ...
                    <div className="fixed top-0 right-0 h-full w-full sm:w-[480px] bg-white shadow-2xl z-[100] transform transition-transform duration-300 ease-in-out translate-x-0 animate-in slide-in-from-right flex flex-col font-sans">
                        ...
                        <form onSubmit={handleProfileSave} className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                            ...
                            <div className="pt-6 border-t border-red-100 bg-red-50/30 rounded-xl p-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-xs font-semibold text-red-700">Danger Zone</p>
                                        <p className="text-[10px] text-red-600 mt-0.5">Once you delete your account, there is no going back. Please be certain.</p>
                                    </div>
                                    <Button
                                        type="button"
                                        variant="destructive"
                                        className="h-9 text-xs font-bold text-white uppercase tracking-wider bg-red-600 hover:bg-red-700"
            ...
            {/* DELETE ACCOUNT CONFIRMATION MODAL - Sliding Panel */}
            {showDeleteModal && (
                <>
                    {/* Backdrop */}
                    <div
                    ...
                    <div className="fixed top-0 right-0 h-full w-full sm:w-[480px] bg-white shadow-2xl z-[100] transform transition-transform duration-300 ease-in-out translate-x-0 animate-in slide-in-from-right flex flex-col font-sans">
                        ...
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-emerald-50/35">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">Confirm account deletion</p>
                            </div>
                            <button
                                type="button"
                                onClick={() => { setShowDeleteModal(false); setDeletePassword(''); setDeleteError(''); }}
                        ...
                        {/* Scrollable Form */}
                        <form onSubmit={handleDeleteAccount} className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                            <div className="space-y-4">
                                <div className="flex items-center justify-center mb-4">
                                    <AlertTriangle className="w-12 h-12 text-red-600" />
                                </div>
                                <h3 className="text-xl font-bold text-slate-800 text-center">Delete Account Permanently</h3>
                                <p className="text-slate-600 text-center text-sm">
                                    This action cannot be undone. All your data including appointments, patient records,
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className={deleteError ? "border-red-500 focus-visible:ring-red-500 h-10" : "h-10"}
                                        disabled={isDeleting}
                                    />
                            ...
                            <div className="flex gap-3 pt-4 border-t border-gray-100 bg-white sticky bottom-0 z-10 pb-2">
                                ...
                                <Button
                                    type="submit"
                                    className="flex-1 h-11 text-xs font-bold text-white uppercase tracking-wider bg-red-600 hover:bg-red-700"
